...
class LUT_Model_ft(nn.Module):
    ...
    def forward(self, img_lr):
        ...
        lut_tensor = self.weight_lut_msb.T.view(1, 3, 1, 64).to(MSB.device)
        ...
        w_msb_strong = w_msb_strong.to(MSB_out.device)
        w_msb_weak = w_msb_weak.to(MSB_out.device)
        w_msb_medium = w_msb_medium.to(MSB_out.device)
        ...
        lut_tensor = self.weight_lut_lsb.T.view(1, 3, 1, 64).to(LSB.device)
        ...
        w_lsb_strong = w_lsb_strong.to(LSB_out.device)
        w_lsb_weak = w_lsb_weak.to(LSB_out.device)
        w_lsb_medium = w_lsb_medium.to(LSB_out.device)

        LSB_out = w_lsb_strong * LSB_out_3 + w_lsb_weak * LSB_out_2 + w_lsb_medium * LSB_out


        img_out = MSB_out + LSB_out + nn.Upsample(scale_factor=self.upscale, mode='nearest')(img_lr[:, 0:1, :, :].to(MSB_out.device))
        img_out = torch.clamp(img_out, 0, 1)


        return torch.clamp(img_out, 0, 1).to(img_lr.device)
